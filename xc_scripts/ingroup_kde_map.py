...
Edit the paths and settings below, then run:
    python xc_scripts/ingroup_kde_map.py
"""
...
BANDWIDTH_KNN_SCALE = 1  # Multiplier for the "knn" bandwidth.
...
def filter_top_logit_per_recordist(df: pd.DataFrame) -> pd.DataFrame:
...
def plot_map(
    ...
    x_merc, y_merc = point_transformer.transform(lon, lat)
    ...
    output_file(OUTPUT_HTML, title="Ingroup KDE Isoclines")
    show(plot)
    ...
    print(f"[INFO] Output written to {OUTPUT_HTML}.")


def main() -> None:
    """Load data, compute KDE, and render the map."""
